...
interface EmailConfirmationPendingProps {
  email?: string; // lo volvemos opcional
  onSignOut?: () => void;
}
...
export const EmailConfirmationPending: React.FC<EmailConfirmationPendingProps> = ({ 
  ...
  // Obtener el email desde localStorage si no se pasó como prop
  useEffect(() => {
    const savedEmail = localStorage.getItem("pendingEmailVerification");
    setEmail(propEmail || savedEmail);
  }, [propEmail]);
  ...
  const handleSignOut = async () => {
    try {
      ...
      localStorage.removeItem("pendingEmailVerification"); // Limpiar
