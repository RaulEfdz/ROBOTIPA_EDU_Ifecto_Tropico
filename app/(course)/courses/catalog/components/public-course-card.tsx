    ...
    <Card className="group h-full overflow-hidden rounded-xl border bg-white shadow-sm transition-all hover:shadow-lg dark:bg-slate-800 dark:border-slate-700 flex flex-col">
        ...
        <Link href={courseLink} aria-label={`Ver detalles del curso ${title}`}>
          <div className="relative aspect-video w-full overflow-hidden">
            ...
            {categoryName && (
              <Badge
                variant="secondary"
                className="absolute top-3 right-3 bg-white/80 px-2.5 py-1 text-xs font-semibold text-emerald-700 backdrop-blur-sm dark:bg-slate-900/70 dark:text-emerald-300"
              >
                {categoryName}
              </Badge>
            )}
      ...
      <CardContent className="flex flex-1 flex-col p-4">
        <Link href={courseLink} className="flex-1">
          <h3 className="mb-2 line-clamp-2 text-lg font-bold leading-tight text-slate-800 transition-colors group-hover:text-emerald-600 dark:text-slate-100 dark:group-hover:text-emerald-400">
            {title}
          </h3>
          {shortDescription && (
